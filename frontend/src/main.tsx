...
function App() {
  ...
  const [invoiceClaim,setInvoiceClaim] = useState({claimed_reposts:'',claimed_comments:''});
  ...
  const uploadInvoice = () => {
    ...
    if (!invoiceClaim.claimed_reposts.trim() || !invoiceClaim.claimed_comments.trim() || !Number.isInteger(claimedReposts) || claimedReposts < 0 || !Number.isInteger(claimedComments) || claimedComments < 0) { setNotice('Enter claimed reposts and comments before uploading.'); return; }
    const input = document.createElement('input'); input.type = 'file'; input.accept = 'application/pdf,image/*';
    input.onchange = async () => {
      const file = input.files?.[0]; if (!file) return;
      const body = new FormData(); body.append('invoice', file); body.append('claimed_reposts', String(claimedReposts)); body.append('claimed_comments', String(claimedComments));
