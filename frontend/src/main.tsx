...
function App() {
  ...
  const [postForm,setPostForm] = useState({client:'',link:'',repost_quota:0,comment_quota:0,targeting_mode:'random',target_industry:'',target_industries:[] as string[]});
  ...
  const submitSelectedInvoice = async (file?:File) => {
    if (!file) return;
  ...
  const distributePost = async () => {
    ...
    try {
      ...
      setPostForm({client:'',link:'',repost_quota:0,comment_quota:0,targeting_mode:'random',target_industry:'',target_industries:[]}); setNotice('Post distributed.'); await refreshStaff();
    } catch (error:any) { setNotice(error.message); }
  ...
  const togglePostIndustry = (industry:string,checked:boolean) => {
    const target_industries = checked ? [...new Set([...postForm.target_industries,industry])] : postForm.target_industries.filter(i=>i!==industry);
    const max = creators.filter(c=>c.status==='active'&&target_industries.includes(c.industry || '')).length;
    setPostForm({...postForm,target_industries,repost_quota:Math.min(postForm.repost_quota,max),comment_quota:Math.min(postForm.comment_quota,max)});
  };
  ...
  const postEligibleCreators = postForm.targeting_mode === 'industry' ? activeCreators.filter(c=>postForm.target_industries.includes(c.industry || '')) : activeCreators;
  ...
  const render = () => {
    ...
    if (page === 'Distribute Post') return <section><Hero eyebrow="Operations" title="Distribute a post" text="Create real repost and comment tasks for creators." />{notice&&<div className="notice">{notice}</div>}<div className="card form"><label>Client<input value={postForm.client} onChange={e=>setPostForm({...postForm,client:e.target.value})}/></label><label>LinkedIn post link<input type="url" value={postForm.link} onChange={e=>setPostForm({...postForm,link:e.target.value})}/></label><label>Targeting mode<select value={postForm.targeting_mode} onChange={e=>{const mode=e.target.value;const max=mode==='industry'?0:activeCreators.length;setPostForm({...postForm,targeting_mode:mode,target_industry:'',target_industries:[],repost_quota:mode==='all'?max:Math.min(postForm.repost_quota,max),comment_quota:mode==='all'?max:Math.min(postForm.comment_quota,max)})}}><option value="random">Random</option><option value="industry">By industries</option><option value="all">All creators</option></select></label>{postForm.targeting_mode==='industry'&&<div className="field-group"><small>Target industries</small><div className="month-picks">{[...INDUSTRIES,...customIndustries].map(i=><label key={i}><input type="checkbox" checked={postForm.target_industries.includes(i)} onChange={e=>togglePostIndustry(i,e.target.checked)}/>{i}</label>)}</div></div>}<div className="target-capacity"><div><strong>{postEligibleCount}</strong><span>eligible creators</span></div><small>Maximum reposts: {postEligibleCount} · Maximum comments: {postEligibleCount}</small><button className="secondary" type="button" onClick={()=>setPostForm({...postForm,repost_quota:postEligibleCount,comment_quota:postEligibleCount})}>Use max for both</button></div><div className="two"><label>Repost quota<input type="number" min="0" max={postEligibleCount} value={postForm.repost_quota} onChange={e=>setPostQuota('repost_quota',Number(e.target.value))}/></label><label>Comment quota<input type="number" min="0" max={postEligibleCount} value={postForm.comment_quota} onChange={e=>setPostQuota('comment_quota',Number(e.target.value))}/></label></div><button disabled={distributing||postEligibleCount===0} className="primary" onClick={distributePost}><Send size={16}/>{distributing?'Sending...':'Send tasks'}</button></div></section>;
