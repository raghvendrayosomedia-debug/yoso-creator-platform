...
type CreatorProfile = { id:string; name:string; email:string; industry?:string; followers?:number; linkedin_url?:string; bank_account?:string; ifsc?:string; rate_repost:number|null; rate_comment:number|null; status:'active'|'inactive' };
...
function App() {
  ...
  const [rateEdits,setRateEdits] = useState<Record<string,{rate_repost:string;rate_comment:string}>>({});
  ...
  const refreshStaff = async () => {
    ...
    setRateEdits(Object.fromEntries(creatorRows.map(c => [c.id, { rate_repost:String(c.rate_repost ?? ''), rate_comment:String(c.rate_comment ?? '') }])));
  ...
  const saveRates = async (creator:CreatorProfile) => {
    ...
    await apiJson(`/creators/${creator.id}/rates`, { method:'PATCH', body:JSON.stringify({ rate_repost:Number(values.rate_repost), rate_comment:Number(values.rate_comment) }) });
...
function CreatorOnboarding({onComplete}:{onComplete:(body:any)=>Promise<void>}) {
  const [form,setForm] = useState({name:'',industry:'',followers:0,linkedin_url:'',bank_account:'',ifsc:''});
