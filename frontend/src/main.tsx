...
type Invoice = { id:string; month_label:string; month_key?:string; reposts:number; comments:number; claimed_reposts?:number; claimed_comments?:number; claimed_amount?:number|null; amount_check_state?:'match'|'mismatch'|'unchecked'; amount_check_message?:string|null; verification_state?:'verified'|'mismatch'|'unchecked'; verification_message?:string|null; amount:number|null; invoice_url?:string|null; invoice_name?:string|null; bank_match:'match'|'mismatch'|'unchecked'; approval_state:'pending'|'approved'|'rejected'; reject_reason?:string; paid:boolean; paid_on?:string|null; creators?:{name:string;email:string;bank_account?:string;ifsc?:string} };
...
function App() {
  ...
  const uploadInvoice = () => {
    ...
    input.onchange = async () => {
      const file = input.files?.[0]; if (!file) return;
      const body = new FormData(); body.append('invoice', file); body.append('claimed_reposts', String(claimedReposts)); body.append('claimed_comments', String(claimedComments)); body.append('claimed_amount', String(claimedAmount));
  ...
  const downloadBankFile = async () => {
    const month = exportMonth || approvedInvoiceMonths[0] || '';
  ...
  const approvedInvoiceMonths = Array.from(new Set(invoices.filter(i=>i.approval_state==='approved').map(i=>i.month_label)));
